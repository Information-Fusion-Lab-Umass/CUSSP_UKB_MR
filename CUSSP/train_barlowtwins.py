...
def evaluate_model(model, dataloader):
    ...
    for batch in dataloader:
        ...
        pos_prob = F.softmax(prob, dim=1)[:,1]
        preds = F.softmax(prob, dim=1).argmax(dim=1).cpu().numpy().astype(int)
        for _label, _pred, _prob in zip(label.cpu().numpy().astype(int), preds, pos_prob):
            out_df = out_df.append({"label": _label, "pred": _pred, "prob": _prob}, ignore_index=True)
...
def get_datasets(args, figsize=(12,8)):
    ...
    """
    if os.path.isfile(f"{args.data_dir}/{args.patch_type}_stats.csv"):
        stats_df = pd.read_csv(f"{args.data_dir}/{args.patch_type}_stats.csv")
        patch_mean = stats_df.MEAN.to_numpy()
        patch_std = stats_df.STD.to_numpy()
    else:
        patch_mean, patch_std = get_mean_and_std(label_csv.PID, args.patch_type, args.data_dir)
        stats_df = pd.DataFrame({"MEAN": list(patch_mean), "STD": list(patch_std)})
        stats_df.to_csv(f"{args.data_dir}/{args.patch_type}_stats.csv", index=False)
...
def train(args):
    ...
    model = BarlowTwins(**model_kwargs)
