...
from BarlowTwinsMR.BarlowTwins.models import BarlowTwins_Siamese
from BarlowTwinsMR.BarlowTwins.models import ProjectionHead
from BarlowTwinsMR.BarlowTwins.models import OnlineFineTuner_Siamese

from BarlowTwinsMR.BarlowTwins.dataset import LAX_4Ch_patch_Dataset
from BarlowTwinsMR.BarlowTwins.dataset import LAX_4Ch_Siamese_Dataset
...
from BarlowTwinsMR.BarlowTwins.transforms import SiameseTransform

from BarlowTwinsMR.BarlowTwins.loss import BarlowTwinsLoss
from BarlowTwinsMR.BarlowTwins.loss import ContrastiveLoss
...
def get_datasets(args, figsize=(12,8)):
    ...
    train_transform = SiameseTransform(
        train=True, input_height=32, 
        perspective=True, gaussian_blur=False, 
        normalize=lax_4ch_normalize
    )
    
    val_transform = SiameseTransform(
        train=False, input_height=32, 
        disable_augmentation=True,
        normalize=lax_4ch_normalize
    )
    
    train_dataset = LAX_4Ch_Siamese_Dataset(train_csv, f"{args.data_dir}/{args.patch_type}",
                                            transform=train_transform,
                                            es_frame_file=args.es_frame_file,
                                            frame_window=args.frame_window)
    val_dataset = LAX_4Ch_Siamese_Dataset(val_csv, f"{args.data_dir}/{args.patch_type}",
...
def train(args):
    ...
    encoder = get_encoder(args.encoder, args.frame_window, **encoder_kwargs)

    encoder_output_dim = encoder(torch.rand(1,args.frame_window,32,32)).shape[1]
    #print(f"Encoder output dimension: {encoder_output_dim}") 
    print(f"BarlowTwins dimensions : 50x32x32 -> {encoder_output_dim} -> {args.z_dim}")

    online_finetuner = OnlineFineTuner_Siamese(encoder_output_dim=encoder_output_dim, 
    ...
    model = BarlowTwins_Siamese(**model_kwargs)
...
def parse_args(default=False):
    ...
    argparser.add_argument("--loss_weights", type=float, default=[0.0, 0.0, 1.0], nargs='+', help="Loss weights distributed to cross-correlation (barlow), classification, and siamese contrastive loss")
    ...
    argparser.add_argument("--es_frame_file", default=None, type=str, help="Path to the ES frame csv file path.")
    argparser.add_argument("--frame_window", default=50, type=int, help="Number of frames to use for training.")
